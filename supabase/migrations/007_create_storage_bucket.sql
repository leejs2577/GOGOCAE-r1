...
-- Create RLS policies for storage.objects (if not exists)
CREATE POLICY IF NOT EXISTS "Users can view files from accessible requests" ON storage.objects
FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "Users can upload files to their own requests" ON storage.objects
FOR INSERT WITH CHECK (
  bucket_id = 'request-files' AND
  auth.uid() IS NOT NULL
);

CREATE POLICY IF NOT EXISTS "Users can delete files from their own requests" ON storage.objects
FOR DELETE USING (
  bucket_id = 'request-files' AND
  EXISTS (
    SELECT 1 FROM request_files
